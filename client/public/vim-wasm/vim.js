...
(function () {
  const loadPackage = function (metadata) {
    ...
    const PACKAGE_NAME = "vim.data";
    const REMOTE_PACKAGE_BASE = "vim.data";
...
let wasmMemory;
const wasmTable = new WebAssembly.Table({ initial: 869, maximum: 869 + 0, element: "anyfunc" });
let ABORT = false;
let EXITSTATUS = 0;
...
function _environ_get(__environ, environ_buf) {
  const strings = _emscripten_get_environ();
  let bufSize = 0;
...
const ___tm_timezone = (stringToUTF8("GMT", 247888, 4), 247888);
function _tzset() {
...
function _strptime(buf, format, tm) {
  ...
  const DAY_NUMBERS_SUN_FIRST = { SUN: 0, MON: 1, TUE: 2, WED: 3, THU: 4, FRI: 5, SAT: 6 };
  const DAY_NUMBERS_MON_FIRST = { MON: 0, TUE: 1, WED: 2, THU: 3, FRI: 4, SAT: 5, SUN: 6 };
  ...
  if (matches) {
    ...
    const fullDate = new Date(date.year, date.month, date.day, date.hour, date.min, date.sec, 0);
...
var VW = {
  init: function () {
    ...
    let guiWasmResizeShell;
    let guiWasmHandleKeydown;
    let guiWasmHandleDrop;
    let guiWasmSetClipAvail;
    let guiWasmDoCmdline;
    let guiWasmEmsg;
    let wasmMain;
    ...
    const VimWasmRuntime = (function () {
      ...
      VimWasmRuntime.prototype.start = function (msg) {
        ...
        this.perf = msg.perf;
        const willPrepare = this.prepareFileSystem(
          msg.persistent,
          msg.dirs,
          msg.files,
          msg.fetchFiles,
      ...
      VimWasmRuntime.prototype.exportFile = function (fullpath) {
        try {
          const contents = FS.readFile(fullpath).buffer;
      ...
      VimWasmRuntime.prototype.evalJS = function (file) {
        try {
          const contents = FS.readFile(file).buffer;
      ...
      VimWasmRuntime.prototype.main = function (args) {
        ...
        args.unshift("vim");
        const argvBuf = new Uint32Array(args.length + 1);
      ...
      VimWasmRuntime.prototype.preloadFiles = function (files, remoteFiles) {
        for (let _i = 0, _a = Object.keys(files); _i < _a.length; _i++) {
          const fpath = _a[_i];
          try {
          ...
          paths.map(function (path) {
            ...
            return fetch(remotePath)
              .then(function (res) {
                if (!res.ok) {
                  throw new Error(
                    "Response of request to {remotePath} was not successful: " +
                      res.status +
                      ": " +
                      res.statusText,
                  );
      ...
      VimWasmRuntime.prototype.prepareFileSystem = function (
        persistentDirs,
        mkdirs,
        userFiles,
        remoteFiles,
        ...
        const files = ((_a = {}), (_a[dotvim + "/vimrc"] = vimrc), _a);
        Object.assign(files, userFiles);
        FS.mkdir(dotvim);
        for (let _i = 0, mkdirs_1 = mkdirs; _i < mkdirs_1.length; _i++) {
          var dir = mkdirs_1[_i];
          FS.mkdir(dir);
        }
        ...
        for (let _b = 0, persistentDirs_1 = persistentDirs; _b < persistentDirs_1.length; _b++) {
      ...
      VimWasmRuntime.prototype.handleErrorOutput = function () {
        ...
        const sharedBuf = new Uint8Array(
          this.sharedBufs.takeBuffer(STATUS_NOTIFY_ERROR_OUTPUT, bufId),
        );
      ...
      VimWasmRuntime.prototype.handleRunCommand = function () {
        const _a = this.decodeStringFromBuffer(1),
          idx = _a[0],
          cmdline = _a[1];
      ...
      VimWasmRuntime.prototype.handleSharedBufRequest = function () {
        ...
        const _a = this.sharedBufs.createBuffer(size),
          bufId = _a[0],
          buffer = _a[1];
      ...
      VimWasmRuntime.prototype.handleOpenFileWriteComplete = function () {
        const bufId = this.buffer[1];
        const _a = this.decodeStringFromBuffer(2),
          idx = _a[0],
          fileName = _a[1];
      ...
      VimWasmRuntime.prototype.decodeStringFromBuffer = function (idx) {
        ...
        const s = String.fromCharCode.apply(String, chars);
        return [idx, s];
      };
      ...
      VimWasmRuntime.prototype.printPerfs = function () {
        ...
        const entries = performance.getEntriesByType("measure").map(function (e) {
...
const _gui_wasm_handle_keydown = (Module["_gui_wasm_handle_keydown"] = function () {
  return Module["asm"]["Ua"].apply(null, arguments);
});
const _gui_wasm_resize_shell = (Module["_gui_wasm_resize_shell"] = function () {
  return Module["asm"]["Va"].apply(null, arguments);
});
const _gui_wasm_handle_drop = (Module["_gui_wasm_handle_drop"] = function () {
  return Module["asm"]["Wa"].apply(null, arguments);
});
const _gui_wasm_set_clip_avail = (Module["_gui_wasm_set_clip_avail"] = function () {
  return Module["asm"]["Xa"].apply(null, arguments);
});
const _gui_wasm_do_cmdline = (Module["_gui_wasm_do_cmdline"] = function () {
  return Module["asm"]["Ya"].apply(null, arguments);
});
const _gui_wasm_emsg = (Module["_gui_wasm_emsg"] = function () {
  return Module["asm"]["Za"].apply(null, arguments);
});
const _wasm_main = (Module["_wasm_main"] = function () {
